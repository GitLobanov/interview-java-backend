...
JPQL (Java Persistence Query Language) и HQL (Hibernate Query Language) — это объектно-ориентированные языки запросов, которые позволяют работать с сущностями, а не с таблицами базы данных. JPQL и HQL имеют схожий синтаксис и функциональность, поскольку HQL является расширением JPQL для Hibernate. Они оба работают на уровне объектов и сущностей, а не на уровне SQL-запросов.
...
#### Как перехватить изменения в Entity ? []
...
#### 17. Коллекция: `List` vs `Set` в JPA (OneToMany)
